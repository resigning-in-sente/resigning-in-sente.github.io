...
In this article we will introduce the concept of move debt and how it relates to positional evaluation - and join in on the secret scam that is high-level baduk strategy.
...
However, it is something that high-level players will reference here and there, in terms such as "owing a move locally" and such.
...
In the first example we examine a common outcome of the modern 4-4 low approach.
Here White, by its choice to push upward to 1, neglects to directly defend a major weakness (the peep at A) and thus incurs a move debt.
From a *purely local* perspective, White arguably **owes Black a move** defending this weakness, such as A, B, or C.
Failure to defend in a timely fashion could lead to Black calling to **collect the move debt** by peeping at A directly, or approaching from the side threatening A as a follow-up.
Note that this discussion isn't about whether White *should* spend an extra move here in actual play: in fact, with this jōseki White typically does not unless put under more severe pressure.
We are simply noting that White has an undefended weakness, which Black could capitalize on if White does not spend an extra move, independent of whether defending or capitalizing is good play from a global perspective.
...
But since White can only play one side at a time, this time White must go into debt on whichever side White fails to reinforce.
...
The fact that Black owes multiple moves on the board does not mean that Black is in a bad position.
Both White and Black will look to play around these debts: Black to ameliorate their negative consequences, and White to exacerbate them.
...
> **The 2-step guide to being a debt scammer**
> 1. Avoid paying your debts whenever possible.
> 2. Force everyone else to pay their debts.
...
Black will accomplish this by only establishing a skeleton of the territorial boundaries it wants, omitting multiple moves that would normally be required to secure it: thus Black takes on some serious move debt, in this case about five stones' worth.
The goal of this strategy is to avoid being slowly strangled to death, and to force White into a dilemma.
White can:
...
This is the true nature of greedy and fast strategies.
Black takes on significant move debt on purpose, and the game becomes a contest between Black's refusal to pay its debts versus White's struggle to collect or punish Black's debts.
...
Through the last example we saw how move debt emerges naturally in actual play, and how it can turn into a central strategic focal point.
Naturally, it is not the unique framework in which we could have viewed the example.
Move debt is ultimately a single aspect of the broader idea of the strength and weakness of groups, and we could have discussed the above example purely in terms of playing away from strength.
...
Furthermore, move debt is a convenient framework in which to organize our idea of "fast" versus "slow" play.
Fast play is fast by virtue of omitting otherwise natural defenses, thereby incurring move debt: play is slow when it includes the usual defenses and thus avoids incurring move debt.
